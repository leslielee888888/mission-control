...
| R-8 | 2026-09-15 | Even after R-7 removed the *duplicated* credentials doc, the underlying mismatch R-5 hit could still recur: `scripts/seed.py` generated a fresh random password per user on every run (`secrets.token_urlsafe`), so local, CI, and the NAS each got their own independent set — the same user's password was never actually the same in two places, only documented in one place per environment. | Replaced the random per-run password with `_demo_password(email)`, a deterministic SHA-256-based derivation — same email always yields the same password, on any machine, any run, no state shared between environments required. Verified: reseeded locally twice, confirmed identical passwords both times; full test suite (137 tests) still green; reseeded the NAS (`docker compose down -v && pull && up -d`) and confirmed `director.dana@northwind.demo` now logs in with the exact password `seed_credentials.txt` shows locally. | No — same seed-time password-generation contract (§10 #24), just deterministic instead of random; no FR changed. |
