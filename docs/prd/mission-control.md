...
| R-13 | 2026-09-16 | Leslie found the README's "Demo credentials" section still described the pre-R-8 random-per-run password scheme, and pointed reviewers at `seed_credentials.txt` — gitignored, never committed — to find them; a reviewer just browsing the GitHub repo (not cloning/running anything) had nothing to actually read. Leslie's follow-up: since R-8 made the password static and every email in `scripts/seed.py` is already a hardcoded literal, why not just commit the file — it's fully deterministic now, not a stale per-run snapshot. A `/code-review` of that change raised a real High finding: the committed file is a working credential list — director-level access included — for a NAS instance that's actually running, not just synthetic data with no live system behind it. | Verified determinism first (reseeded twice locally, diffed the two `seed_credentials.txt` outputs — byte-for-byte identical), then removed it from `.gitignore` and committed the actual file; updated the README's Demo credentials section to link it directly plus a one-account-per-role table, and fixed `scripts/seed.py`'s own comment and the file's own header (both still said/implied "gitignored"/"generated fresh this run"). Verified all four table accounts against the live NAS. On the High finding: Leslie's explicit call, weighed knowingly — the NAS is LAN-only (§3 non-goals), the data behind every account is synthetic, and this is a take-home demo, not a production system; kept the file committed as-is. | No — docs/data-file only; the credential-generation logic itself is unchanged from R-8. |
