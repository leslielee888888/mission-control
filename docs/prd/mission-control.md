...
| R-13 | 2026-09-16 | Leslie asked to connect this repo to his personal PR-review-dashboard tooling (a separate, self-hosted project — see that repo's own history for its side of this) so PRs here get an automated second review alongside `/code-review`. The dashboard's default poller (~3 min cycle) already covered it once the repo was added to its allow-list; a faster GitHub-Actions-triggered path (the exact pattern already used on Leslie's `ai-docs`/`leslie-awesome-skills` repos) was copied over next — but those are *private* repos, where a self-hosted LAN runner triggered by plain `pull_request` is a safe, established pattern. mission-control is **public**, and that same pattern is a known arbitrary-code-execution vector: for `pull_request`, GitHub evaluates the workflow file from the PR's own head ref, so a fork PR could rewrite this file's steps and run them on Leslie's LAN-connected runner. Missed that distinction copying the pattern over; caught it via the dashboard's own `/code-review` of the PR that added it. | Added `.github/workflows/pr-review-trigger.yml` (fires the dashboard's `POST /api/trigger` on `opened`/`synchronize`/`ready_for_review`, `continue-on-error: true`, never a required check — the poller is the real fallback if it fails). Closed the security gap two ways: (1) the actual fix — set this repo's "Fork pull request workflows" policy to `all_external_contributors` (Settings → Actions → General), a platform-level control evaluated by GitHub before any workflow file is even read, so a fork can't edit past it the way it could an in-file condition; (2) `head.repo.full_name == github.repository` in the workflow's own `if:` guard, kept as cheap defense in depth on top of (1), not instead of it. Verified live: opened a real PR, watched the trigger fire, the dashboard's runner pick it up, and a review complete — end to end, not just "the workflow file parses." | No — infra/tooling only, no FR touched; the security finding was about *how* to wire this up safely, not a change to what Mission Control itself does. |
